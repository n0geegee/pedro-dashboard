# 00 — PROMPT DLA PEDRO START HERE

To jest plik startowy do wklejenia/uruchomienia na iMac-Hermes. Zacznij od tego pliku, nie od luźnych wiadomości z Discorda.
...
ŹRÓDŁO PRAWDY
Najpierw przeczytaj plan projektu. Na Pedro/iMac-Hermes właściwy lokalny root projektu to:

/home/imac-hermes/projects/pedro_dashboard

Szukaj lokalnie w tej kolejności:
1. /home/imac-hermes/projects/pedro_dashboard
2. ~/projects/pedro_dashboard
3. /linus1/hermes/projects/pedro_dashboard
4. ~/hermes/projects/pedro_dashboard
5. ~/.hermes/projects/pedro_dashboard
6. jeśli brak — znajdź katalog pedro_dashboard pod /home/imac-hermes, /linus1, ~/ lub ~/.hermes.

Pracuj z lokalnej kopii na Pedro, nie z cudzej ścieżki/mounta, chyba że Jurand wyraźnie poprosi o sync z Dell/linus1.
...
- docs/visual_target_v1_from_jurand_screenshot.md jeśli istnieje — docelowy wygląd z referencji Juranda; ma pierwszeństwo dla UI polish nad ogólnym 2×2 skeletonem
- docs/host_inventory.md / docs/mvp_task_list.md jeśli już istnieją
...
FIRST-RUN GUARD — ŻEBY DOWIEŹĆ MVP ZA PIERWSZYM RAZEM
- Zacznij od najmniejszej pionowej ścieżki: `python server -> /health -> index.html -> state/*.json -> polling UI`.
- Jeśli brakuje zależności, preferuj standard library i małe skrypty; nie wprowadzaj frameworka, dopóki MVP może działać bez niego.
- Każdy etap ma kończyć się realnym uruchomieniem albo konkretnym blockerem z outputem komendy.
- Nie rób równocześnie estetyki, adapterów danych, audio i ops. Najpierw działający ekran, potem kontrakty JSON, potem no-systemd ops, potem QA.
- Po pierwszym działającym skeletonie zrób screenshot/visual check 1920×1200 lub jasno zapisz, dlaczego nie dało się go wykonać na hoście.
- Jeżeli delegujesz workerom, dawaj im wąskie zadania i zakaz edycji tych samych plików.
